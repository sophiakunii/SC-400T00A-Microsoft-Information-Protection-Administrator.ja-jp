...
1. **[ポリシーを適用する場所を選択する]** 領域で、Exchange メール、SharePoint サイト、OneDrive アカウント、Microsoft 365 グループが選択されていることを確認し、**[次へ]** を選択します。
...
1. **[Teams チャット]** ペインで、次のユーザーを追加します。 
    - Adele Vance
    - Pradeep Gupta
...
### <a name="task-4--create-retention-policy-with-adaptive-scope"></a>タスク 4 – アダプティブ スコープを使用してアイテム保持ポリシーを作成する
...
1. **[アダプティブ ポリシー スコープの名前を設定]** ページの **[名前]** と **[説明]** に、次の情報を入力します。

    - **[名前]** : 法的ドキュメントの保持
    - **[説明]** : 法律に関連するすドキュメントの保持

1. **[次へ]** ボタンを選択します。
1. **[作成するスコープの種類]** ページで、 **[ユーザー]** を選択します。
1. **[次へ]** ボタンを選択します。
1. **[ユーザーを定義するクエリの作成]** ページの **[ユーザー属性]** で、属性のドロップダウン メニューを開き、 **[部署]** を選択します。
1. 属性フィールドのすぐ横で、演算子として **[が次の値に等しい]** を選択します。
1. 値として **[法務]** を選択します。
1. 2 つ目の属性を追加するには、 **[ユーザーを定義するクエリの作成]** ページで **[+ 属性を追加]** を選択します。
1. **[クエリ演算子]** 、 **[属性]** 、 **[演算子]** 、 **[値]** に、次の情報を入力します。
    - **クエリ演算子**:もしくは
    - **属性**:部署
    - **演算子**: が次の値に等しい
    - **値**: 小売
...
1. **[次へ]** ボタンを選択します。
1. **[確認と完了]** ページで、**[送信]** ボタンを選択します。
1. 送信が成功したら、 **[完了]** ボタンをクリックしてページを閉じます。
...
2. 新しい **[アダプティブ ポリシー スコープの選択]** ページで、 **[法的ドキュメントの保持]** を選択し、 **[追加]** ボタンを選択します。

1. **[ポリシーを適用する場所の選択]** で、次の情報を入力します。
...
1. ポリシーが作成されたら、**[完了]** を選択します。
### <a name="task-5--test-adaptive-scope-policy"></a>タスク 5 – アダプティブ スコープ ポリシーをテストする

この演習では、アダプティブ スコープの影響を受けるユーザーを確認し、新しいアダプティブ アイテム保持ポリシーをテストします。

注: 注: アイテム保持ポリシーを作成して送信すると、アイテム保持ポリシーが適用されるまでに最大 7 日かかることがあります。
...
1. **[データ ライフサイクル管理]** ページの **[アダプティブ スコープ]** タブで、 **[法的ドキュメントの保持]** を選択します。

1. 新しい **[法的ドキュメントの保持]** ページで、 **[スコープの詳細]** を選択します。
1. **[法的ドキュメントの保持]** ページで、スコープの影響を受けるすべての場所を確認できます。

[//]: <> (ラボ テナントのエラー:データの読み込みに失敗しました。もう一度やり直してください)

1. アダプティブ スコープ アイテム保持ポリシーの詳細を確認するには、マウスの右ボタンで Windows ボタンを選択して PowerShell ウィンドウを開き、[Windows PowerShell] を選択します。
