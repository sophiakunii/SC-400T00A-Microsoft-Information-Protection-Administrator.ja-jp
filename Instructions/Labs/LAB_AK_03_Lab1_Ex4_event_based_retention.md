...
1. 左側のナビゲーション ウィンドウで **[個人用サイト]** を選択し、右側の **[頻繁に利用するサイト]** で **[ブランド]** を選択します。

1. 上部のナビゲーション ウィンドウで **[ドキュメント]** を選択します。

1. **[ドキュメント]** ページで、**Customer Product Survey.xlsx** の前にあるチェック ボックスをオンにして選択したら、3 つの縦方向のドットを選択します。
...
1. 新しく表示された **[資産 ID]** フィールドを **NewProductLaunch** に設定し、右上隅の **[X]** を選択して、右側のメニューを閉じます。
...
1. **[イベントの設定]** ページで、 **[SharePoint と OneDrive のアイテムの資産 ID]** を **NewProductLaunch** に設定して、今日の日付を選択します。

1. **[次へ]** を選び、 **[送信]** を選んでから、 **[完了]** を選びます。

イベントが正常にトリガーされ、"プロジェクト資産" ラベルと資産 ID NewProductLaunch が設定されたすべてのドキュメントの保持期間を開始しました。

## タスク 6 – イベント トリガーの結果を観察する
