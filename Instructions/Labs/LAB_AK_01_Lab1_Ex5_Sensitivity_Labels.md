...
9. **[アカウントにサインイン]** フォームで、**Joni Sherman** JoniS@WWLxZZZZZZ.onmicrosoft.com としてサインインします (ZZZZZZ はラボ ホスティング プロバイダーから支給された固有のテナント ID)。  Joni のパスワードは、ラボ ホスティング プロバイダーから支給されます。
...
14. **[アカウントにサインイン]** フォームで、**MOD 管理者** としてサインインします。 admin@WWLxZZZZZZ.onmicrosoft.com (ZZZZZZ はラボ ホスティング プロバイダーから支給された固有のテナント ID)。  管理者のパスワードは、ラボ ホスティング プロバイダーから支給されます。
...
### <a name="task-2--create-sensitivity-labels"></a>タスク 2 – 秘密度ラベルを作成する
...
3. **Microsoft 365 コンプライアンス** ポータルの左ナビゲーション ペインで、 **[情報の保護]** を選択します。  

4. 「情報保護」ページの「**ラベル**」タブで、「 **+　ラベルを作成**」を選択します。

5. **「新しい秘密度ラベル」** ウィザードが起動します。 **「ラベルのツールチップの名前を付け作成する」** ページで、 **「名前」** 、 **「管理者向けの説明」** 、 **「ユーザー向けの説明」** に次の情報を入力します。
...
12. **[Auto-labeling for schematized data assests(preview)]\(スキーマ化されたデータ アセット (プレビュー) の自動ラベル付け\)** ページで、 **[次へ]** を選択します。 
...
16. [情報の保護] ページで、新しく作成した **[internal]** ラベルを (選択せずに) 強調表示し、縦に並んだ **[...]** を選択します。

17. ドロップダウン メニューで **「+ サブ ラベルの追加」** を選択します。

18. **「新しい秘密度ラベル」** ウィザードが起動します。 **「ラベルのツールチップの名前を付け作成する」** ページで、 **「名前」** 、 **「管理者向けの説明」** 、 **「ユーザー向けの説明」** に次の情報を入力します。
...
20. **「このラベルのスコープを定義」** ページで、 **「ファイルと電子メール」** オプションを選択します。 

21. **[次へ]** を選択します。

22. **[ファイルとメールの保護設定を選択]** ページで、 **[ファイルとメールを暗号化]** オプションをオンにします。
...
31. **「グループとサイトの保護設定を選択」** ページで、 **「次へ」** をクリックします。

32. **[Auto-labeling for schematized data assests(preview)]\(スキーマ化されたデータ アセット (プレビュー) の自動ラベル付け\)** ページで、 **[次へ]** を選択します。 

33. **「設定の確認と終了」** ページで、 **「ラベルの作成」** を選択します。
...
### <a name="task-3--publish-sensitivity-labels"></a>タスク 3 – 秘密度ラベルを発行する

次に、Internalと人事の秘密度ラベルを発行します。これにより、発行された秘密度ラベルを人事ユーザーが人事ドキュメントに適用できるようになります。
...
4. 「**情報の保護**」ページの「**ラベル**」タブで、「**ラベルを発行**」を選択します。

5. 「秘密度ラベルの発行」ウィザードが起動します。

6. **「発行する秘密度ラベルを選択」** ページで、 **「リンクを発行する秘密度ラベルを選択」** を選択します。
...
14. **「電子メールに既定のラベルを適用する」** ページで **「次へ」** を選択します。

15. **[Power BI コンテンツにこの既定のラベルを適用する (プレビュー)]** ページで、 **[次へ]** を選択します。
...
### <a name="task-4--work-with-sensitivity-labels"></a>タスク 4 – 秘密度ラベルを使って作業する
...
2. **Microsoft Edge** に、Microsoft 365 コンプライアンス センターのタブがまだ開かれているはずです。 アドレス バーを選択し、 **https://portal.office.com** に移動します。
...
8. 上部ペインの上部にある **[秘密度]** を選択し、ドロップダウン メニューを開きます。 **「内部」** を選択してラベルを適用します。
...
6. **「新しい秘密度ラベル」** ウィザードが起動します。 「ラベルのツールチップに名前を付け作成する」ページで、次の情報を入力します。
...
8. **「このラベルのスコープを定義」** ページで、 **「ファイルと電子メール」** オプションを選択します。

9. **[次へ]** を選択します。

10. **「ファイルと電子メールの保護設定を選択」** ページで、 **「次へ」** をクリックします。

11. **「ファイルと電子メールの自動ラベル付け」** ページで、 **「ファイルと電子メールの自動ラベル付け」** を有効に設定します。

12. **「これらの条件に一致するコンテンツを検出する」** セクションで、 **「+条件の追加」** を選択し、 **「コンテンツに含まれている」** を選択します。

13. **「コンテンツに含まれている」** セクションで、 **「テキストの追加」** を選択し、 **「機密情報の種類」** を選択します。

14. 右側に **「機密情報の種類」** パネルが表示されます。
...
17. **「すべて選択」** チェック ボックスをオンにします。

18. **[追加]** を選択します。

19. **[次へ]** を選択します。

20. **「グループとサイトの保護設定を選択」** ページで、 **「次へ」** をクリックします。

21. **[Auto-labeling for schematized data assests(preview)]\(スキーマ化されたデータ アセット (プレビュー) の自動ラベル付け\)** ページで、 **[次へ]** を選択します。 
...
25. **[情報の保護]** ページで、 **[ラベルを発行]** を選択します。

26. 「秘密度ラベルの発行」ウィザードが起動します。

27. **「発行する秘密度ラベルを選択」** ページで、 **「リンクを発行する秘密度ラベルを選択」** を選択します。

28. 右側には、 **「発行する秘密度ラベル」** というサイド バーが表示されます。

29. **「内部」** と **「内部/GDPR ドイツ」** チェックボックスを選択します。

30. **[追加]** を選択します。

31. **「発行する秘密度ラベルを選択」** ページで **「次へ」** を選択します。

32. **「ユーザーとグループに発行」** ページで **「次へ」** を選択します。

33. **「ポリシー設定」** ページで **「次へ」** を選択します。

34. **「ドキュメントに既定のラベルを適用する」** ページで **「次へ」** を選択します。

35. **「電子メールに既定のラベルを適用する」** ページで **「次へ」** を選択します。

36. **[Power BI コンテンツにこの既定のラベルを適用する (プレビュー)]** ページで、 **[次へ]** を選択します。 

37. **「ポリシーの名前を設定」** ページで、次の情報を入力します。
...
39. **「確認と完了」** ページで、 **「送信」** を選択します。

40. ポリシーが作成され、完了すると **「新しいポリシーが作成されました」** というメッセージが表示されます。
...
## <a name="you-have-completed-the-lab-1-proceed-to-lab-2---exercise-1"></a>これでラボ 1 は完了です。 ラボ 2 - 演習 1 に進む
